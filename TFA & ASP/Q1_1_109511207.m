...
for ii= 1: 1 :Nt  % for every ii transmit element
ii
...
title('Phased array imaging - with envelope detection')
axis equal

%%%% Without Envelope Detection
%%% Log compression
bf_nor = abs(bf)/max(max(abs(bf)));  % normalized to 1.
srcInt2 = 20*log10(bf_nor);

figure
pcolor(X*1e3, Z*1e3, srcInt2+DR )
axis('ij');
shading interp;
caxis([0 DR])
colorbar
...
%%% save envelope detection result
save("ori_img.mat", "bf_env_nor")
